      ...
      const subscription = downloadService.startPollingByRequestToken(downloadToken).subscribe({
        next: statusObj => {
          if (statusObj?.status === 'completed' && statusObj?.file_path) {
            const filePath = `${globalConstants.DATA_DOWNLOAD_STATUS_PREFIX}/${statusObj.file_path}`;
            const fileUrl = DATA_DOWNLOAD_BASE_URL ? `${DATA_DOWNLOAD_BASE_URL}${filePath}` : filePath;
          ...
          <div className={[`${header}`, `${downloadError ? error : ''}`].join(' ')}>
            <h1 data-testid="header">
              {downloadError
                ...
                : checking
                ? downloadPageTextContent.checking
