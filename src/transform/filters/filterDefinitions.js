...
const filters = [
  ...
  },
  {
    id: 'pdf',
    matcher: (dataset) => {
      return dataset.dataFormats
        ? dataset.dataFormats.some(dataFormat => dataFormat === 'PDF')
        : false;
    },
    groupId: 'dataFormat',
    label: 'Reports (PDF)',
  ...
  },
  {
    ...
    matcher: (dataset) => {
      return dataset.dataFormats
        ? dataset.dataFormats.some(dataFormat => dataFormat === 'XLS')
        : false;
    },
    groupId: 'dataFormat',
    label: 'Reports (Excel)',
  ...
  // Override auto-creation for RSS due to hard-coded 'Management/CFO'.
  {
    id: 'RSS',
    matcher: (dataset) => {
      return (dataset.publisher === "Retail Securities Services"
        || dataset.publisher === 'Management/CFO');
    },
    groupId: 'publisher',
...
const filtersIndexedById = (filters) => filters.reduce((acc, filter, i) => {
  acc[filter.id] = i;
  return acc;
}, {})
...
const processFilters = (dataset) => {
  ...
  dataset.filters = filters.filter(fil => fil.matcher(dataset))
    .map(applicableFilter => applicableFilter.id);
};

const sortPublishers = (availableFilters) => {
  availableFilters = partialSort(availableFilters, 'groupId',
    'publisher',
    (a, b) => {
...
const addMissingPublisher = (datasetToCheck) => {
  ...
  if (filters.filter(filter => filter.groupId === 'publisher')
    .some(filter => filter.matcher(datasetToCheck))) {
    return;
  }
...
const filterByDateRange = (params) => {
  const latestDate = parse(
    params.dataset.techSpecs.latestDate,
    "MM/dd/yyyy",
    new Date()
  ).setHours(12);
  const earliestDate = parse(
    params.dataset.techSpecs.earliestDate,
    "MM/dd/yyyy",
    new Date()
  ).setHours(12);
