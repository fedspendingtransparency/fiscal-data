...
export const updateProgress = (download, onChangeCallback) => {
  ...
  if (download.pagesProcessed) {
    const processed = download.pagesProcessed < download.totalPages
      ? download.pagesProcessed
      : download.totalPages;
  ...
  if (!download.tablesCompleted || download.tablesCompleted < tableCount) {
    ...
    const tableCompletionAdjustment = aveProcPerPage * pagePerTableFactor * (
      (download.totalPages / tableCount) ** pagePerTableExponent
    );
    ...
    let allArchiving = (
      (
        ...
        (aveElapsedPerPage / (
          download.totalPages - (
            archivePageFactor * download.totalPages
          )
        )
      ) * (
        (download.totalPages - 1) ** archiveExponent
      ))
    );
    ...
    if (download.tableCount > 1) {
      ...
      // 10000 record pages
      allArchiving += (allArchiving * (
        archivePagePerTableFactor * (download.totalPages / download.tableCount)
      ));
    ...
    download.estimatedFinishTime /= (
      download.tablesStarted ? (download.tablesStarted + 1) / (tableCount + 1) : 0.7
    );

    // don't let progress get too close to 100% before the last table has been completed
    download.estimatedFinishTime = download.estimatedFinishTime > (elapsed * 1.05)
      ? download.estimatedFinishTime
      : (elapsed * 1.05);
    ...
    if (download.tablesCompleted === tableCount) {
      ...
      let allArchiving = ((elapsed ** timingExponent) * archiveFactor) + (
        archivePageFactor * (
          (aveElapsedPerPage / (
            download.totalPages - (
              archivePageFactor * download.totalPages
