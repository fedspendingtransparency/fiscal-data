...
const purgeCharactersNotInCharSet = (
  ...
  return value
    ...
    .map((char: string, charIndex: number, srcArray: string[]): string => {
      ...
      if (preserveDecimal && charIndex === srcArray.length - (precision + 1) && char === '.') return char;
...
const SplitFlapDisplay = ({
  ...
  const updateCycle = () => {
    ...
    const nextValue = finalChars
      .map((char: string, valuePosition: number, srcArray: string[]) => {
        ...
        if (showCommas && char === ',') return char;
        if (valueType !== 'integer' && char === '.' && valuePosition === srcArray.length - (precision + 1)) {
          return char;
        }
  ...
  const shortenedNumberWithMagnitude = (fullNumber: string, sigFigs: number): { value: string; magnitude: string } => {
  ...
  const setDisplayChars = (minimumLength: number): void => {
    ...
    if (padDirection === 'left' && minLength > 3 && !currValue.includes(' ')) {
      setPrevChars([...minLengthFill(prevValue, charSet, minimumLength), ...prevValue.split('')]);
      setCurrChars([...minLengthFill(currValue, charSet, minimumLength), ...currValue.split('')]);
    } else if (minLength > 3 && !currValue.includes(' ')) {
      setPrevChars([...prevValue.split(''), ...minLengthFill(prevValue, charSet, minimumLength)]);
      setCurrChars([...currValue.split(''), ...minLengthFill(currValue, charSet, minimumLength)]);
    }
  };

  const initializeCharArrays = (initLength: number): void => {
    const initValue: string = Array(initLength)
      .fill(charSet[0])
      .join('');
  ...
  useEffect(() => {
    ...
    setDisplayValue(formatNumberForDisplay(shortenDisplay ? shortenedValue : originalValue.current));
  }, [value]);
  ...
  return (
    <div className={splitFlapContainer} role="img" aria-label={`$${description}`}>
      {displayReady && (
        <>
          <div className={flapContainer} aria-hidden={true}>
            {prevChars && <span className={currency}>$</span>}
            {prevChars &&
              prevChars.map((prevChar: string, index: number) => (
                <React.Fragment key={`split-flap-char-${index}`}>
                  <SplitFlapCharacter
                    cycleDelay={stepCycleDelay}
            ...
            {displayValue && displayValue.length && (
              <div className={selectableDigits} data-testid="selectable-digits">
                ...
                {displayValue.split('').map((valChar: string, index: number) => (
                  <span className={[',', '.'].includes(valChar) ? punctuation : spacedDigit} key={index}>
                    {valChar}
                  </span>
                ))}
              </div>
          ...
          {shortenDisplay && (
            <>
              ...
              <div>&nbsp;</div>
              <div className={magnitude}>{displayMagnitude}</div>
              <div>&nbsp;</div>
            </>
          )}
        </>
