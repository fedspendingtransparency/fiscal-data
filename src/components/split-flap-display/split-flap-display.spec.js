...
jest.mock('./split-flap-character/split-flap-character', function() {
  return {
    __esModule: true,
    default: jest.fn().mockImplementation(({ cycleDelay, value, prevValue }) => (
      <span>
        `${value}:${prevValue}`
      </span>
    )),
...
describe('Split Flap Display ', () => {
  it('starts with number of positions', () => {
    const { getAllByText } = render(<SplitFlapDisplay value={'10'} minLength={5} />);
  ...
  it('shows shortened number when width less than breakpoint', () => {
    const { getByTestId, getByText } = render(<SplitFlapDisplay value={'10000000.00'} mobilePrecision={2} minLength={21} />);
