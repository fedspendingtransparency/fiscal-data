...
import HomeHighlightCards, {
  baseFontSize,
  cardWidthRem
} from './home-highlight-cards';
...
describe('Home Highlight Cards', () => {
  ...
  it('hides the desired number of cards based on screen dimensions', () => {
    ...
    jest.spyOn(containerDiv, 'clientWidth', 'get')
      .mockReturnValue(numVisibleCards * cardWidth);
    act(() => {
      global.window.dispatchEvent(new Event("resize"));
      jest.runAllTimers();
    });

    expect(queryAllByText('Highlight Card').length)
      .toStrictEqual(numVisibleCards);

    // zero hidden because we not rendering them to the dom.
    expect(queryAllByText('Hidden Card').length)
      .toStrictEqual(0);
  });
