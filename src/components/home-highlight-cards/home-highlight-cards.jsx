...
import { cardsContainer, cardWrapper, container, loading, toggleButton } from './home-highlight-cards.module.scss';
...
const HomeHighlightCards = ({ width }) => {
  ...
  const [visibleCardsNum, setVisibleCardsNum] = useState(
    1
  ); /* Set to the lowest number
                                                                         (mobile) to prevent
  ...
  const handleToggle = () => {
    ...
    setExpandedStyle(prevState => (Object.keys(prevState).length === 0 ? baseCollapsedStyle : {}));
  };
  ...
  return (
    <div data-testid="highlight-cards-parent" className={container} ref={containerRef}>
      {highlights.length ? (
        <Grid container className={cardsContainer} style={expandedStyle} data-testid="cards-container">
          {highlights
            .filter((d, i) => i < visibleCardsNum)
            .map((dataset, i) => (
              <Grid className={cardWrapper} item xs={12} sm={6} xl={3} key={i}>
                <HomeHighlightCard cardId={`homepageCard-${i}`} className={cardWrapper} hidden={i > visibleCardsNum - 1} dataset={dataset} />
              </Grid>
            ))}
        </Grid>
      ) : (
      ...
      <button onClick={handleToggle} className={toggleButton} data-testid="collapse-button">
        {buttonContent}
      </button>
    </div>
  );
