 ...
 * @param res
 */
export const transformAPI130 =
  (res: { data: any[], links: Record<string, never>, meta: Record<string, never> }) : any[] => {

  const combinedValueForYear = (yearMonth, array) => {
    return array.filter((e) => `${e.record_calendar_year}${e.record_calendar_month}` === yearMonth)
  ...
  res.data.filter((e) => `${e.record_calendar_year}${e.record_calendar_month}` !== currentYearMonth)
    .forEach((e) => {
    ...
    if (!countByMonth[yearMonth])
      countByMonth[yearMonth] = 0;

    countByMonth[yearMonth]++;

    if (countByMonth[yearMonth] === 2) {
      valuesByMonth[yearMonth] =
        combinedValueForYear(yearMonth, data);
...
export const transformAPI146 =
  (res: { data: any[], links: Record<string, never>, meta: Record<string, never> }) : any[] => {
    ...
    const sortedData = res.data.slice()
      .sort((a, b) => (
        `${a.record_calendar_year}${a.record_calendar_month}`
          .localeCompare(`${b.record_calendar_year}${b.record_calendar_month}`
      )));
    const currentYear = Number(new Date(Date.now()).getFullYear());
    sortedData.filter(r => Number(r.record_calendar_year) < currentYear).forEach(row => {
    ...
    Object.entries(dataByYear).forEach(([yearKey, rates]) => {
      data.push({
        ...
        avg_interest_rate_amt: (rates.reduce((a, b) => (
          Number(a) + Number(b)
        )) / rates.length).toFixed(3)
      });
    });
...
const datasets: IDatasetChartConfigs = [
  ...
  },
  {
    ...
    "data": {
      ...
      "index": "record_calendar_year",
      "filters": [{ // Add filters that translate to the API calls' &filters params.
