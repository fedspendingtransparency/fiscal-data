...
const experimentalTesting = (apis) => {
  ...
  const dummyText = `"Lorem ipsum dolor sit amet, [Monthly Treasury Statement (MTS)]
      (https://fiscaldata.treasury.gov/datasets/monthly-treasury-statement/) consectetur
  ...
  const textLen = dummyText.length;

  apis.forEach(api => {
    // Don't overwrite any existing notes and known limitations, even if experimental is turned on
    if (!api.apiNotesAndLimitations) {
      // Ensure no problems are seen if only half of the data tables create an accordion.
      if (cntr % 2 === 0) {
        // Stagger the accordion text blob for each accordion
        const curTextLen = Math.min(Math.ceil(Math.random() * textLen), textLen);
        api.apiNotesAndLimitations = dummyText.slice(0, curTextLen);
      }
      cntr++;
    }
  });
