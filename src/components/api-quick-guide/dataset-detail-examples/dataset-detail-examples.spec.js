...
describe('Dataset Detail Examples, no response', () => {
  ...
  beforeEach(() => {
     rendered = render(
        <DatasetDetailExamples isAccordionOpen={false} selectedTable={selectedTable} />
     );
...
describe('Example Response', () => {
  ...
  it('displays the content of the API response', async() => {
    const exampleResponse = {data: [{record_date: "2020-03-01", unstripped_amt: "123456789.00"}],
      meta: {record_date: "DATE", unstripped_amt: "CURRENCY"}};
    fetchMock.mockResponse(JSON.stringify(exampleResponse));

    rendered.rerender(
      <DatasetDetailExamples isAccordionOpen selectedTable={selectedTable} />
    );
  ...
  it('shows the proper message when the API call fails', async() => {
    ...
    rendered.rerender(
        <DatasetDetailExamples isAccordionOpen selectedTable={{}} />);
  ...
  it('only calls the API once if multiple useEffect triggers take place simultaneously to ' +
    'call the fetchAPI', async() => {
    // first open the accordion without changing the table...
    rendered.rerender(
      <DatasetDetailExamples isAccordionOpen selectedTable={{endpoint: 'Ignored table'}} />);
    ...
    await act(() => {
      ...
      rendered.rerender(
        <DatasetDetailExamples
          isAccordionOpen
          selectedTable={{endpoint: 'Another ignored table'}}
        />);
      rendered.rerender(
        <DatasetDetailExamples
          isAccordionOpen
          selectedTable={{endpoint: 'Finally a...oh wait...this table is also ignored'}}
        />);
      rendered.rerender(
        <DatasetDetailExamples isAccordionOpen selectedTable={{endpoint: 'NOT IGNORED'}} />);
    });
  ...
  it('sets API response if API call is not canceled and it has a "data" field, sets error ' +
    'response if successful response does not have "data" field', async () => {
    ...
    fetchMock.mockResponses([async() =>
       await new Promise(resolve => setTimeout(() =>
         resolve(JSON.stringify({data: 'This is bad data',
           meta: 'This is bad meta'})), 100)), {}]
  ...
  it('prevents setting an error response if the loadCanceled is triggered before the ' +
    'response returns', async () => {
    fetchMock.mockResponses([async() =>
      await new Promise(res => setTimeout(() =>
        res(new Error('invalid-json')), 100)), {}])
