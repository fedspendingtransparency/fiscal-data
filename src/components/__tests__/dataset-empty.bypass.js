...
describe('Dataset detail', () => {
  it('display a notice within the dataset preview section when API data is empty array', async () => {
    fetchMock.get(
      `begin:http://localhost:3000/api/fiscal_service/v1/accounting/od/savings_bonds_pcs?fields=reporting_date`,
      responseMockData['noDataPrelimResponse']
    );
    fetchMock.get(
      `begin:http://localhost:3000/api/fiscal_service/v1/accounting/od/savings_bonds_pcs?fields=series_cd`,
      responseMockData['noDataPrelimResponse']
    );
    let component = renderer.create();
    await renderer.act(async () => {
      component = await renderer.create(<DatasetDetail pageContext={pageContextMock} />);
    });
  ...
  it('show data with no notice within the dataset preview section when API data array has at least one row', async () => {
    fetchMock.get(
      `begin:http://localhost:3000/api/fiscal_service/v1/accounting/od/savings_bonds_pcs_onerow?fields=reporting_date`,
      responseMockData['oneRowPrelimResponse']
    );
    fetchMock.get(
      `begin:http://localhost:3000/api/fiscal_service/v1/accounting/od/savings_bonds_pcs_onerow?fields=series_cd`,
      responseMockData['oneRowInformedResponse']
    );
    let component = renderer.create();
    await renderer.act(async () => {
      component = await renderer.create(<DatasetDetail pageContext={pageContextMockOneRow} />);
    });
    const instance = component.root;
    const dataDisplay = instance.findByProps({ id: 'data-display' });
    const messages = dataDisplay.findAllByProps({ className: 'message' });
