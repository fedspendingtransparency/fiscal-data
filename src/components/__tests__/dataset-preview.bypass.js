...
describe('DatasetPreview', () => {
  const tree = renderer.create(
    <DatasetPreview shortDescription={'Short description goes here'}
                    requestUrl={'https://bla.bla.bla/hello/there?fields=blabla'}/>
  );
  ...
  it('renders an H2 element containing a default value when no title is supplied through props', () => {
    const titledInstance = renderer.create(<DatasetPreview sectionTitle={'Preview of Awesome Data'}
                    requestUrl={'https://bla.bla.bla/hello/there?fields=blabla'}/>).root;
