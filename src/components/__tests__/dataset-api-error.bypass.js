...
describe('Dataset detail', () => {
  it('displays an error notice within when API response returns a non-200 status code', async () => {
    fetchMock.mock(`begin:http://localhost:3000/api/fiscal_service/v1/accounting/od/savings_bonds_pcs-error-status?fields=reporting_date`, 400, {
      repeat: 2,
    });

    let component = renderer.create();
    await renderer.act(async () => {
      component = await renderer.create(<DatasetDetail pageContext={pageContextMockForErrorStatus} />);
    });
  ...
  it('displays an error notice within when API response returns a 200 code but has a header that indicates a content-type other than json', async () => {
    fetchMock.get(
      `begin:http://localhost:3000/api/fiscal_service/v1/accounting/od/savings_bonds_pcs-html-content?fields=reporting_date`,
      new Response('some html or text response body', { status: 200, headers: { 'Content-Type': 'text/html' } })
    );

    let component = renderer.create();
    await renderer.act(async () => {
      component = await renderer.create(<DatasetDetail pageContext={pageContextMockForWrongContentType} />);
    });
    const instance = component.root;
    const dataDisplay = instance.findByProps({ id: 'data-display' });
