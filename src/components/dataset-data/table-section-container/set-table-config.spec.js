...
describe('set table config', () => {
    ...
    it('sets the config for pivot tables and only includes the intended columns, ' +
      'including record_date when not aggregated', () => {
        const config = setTableConfig(mockConfig, selectedTableLessFields, selectedPivot,
          mockApiPivotData);
  ...
  it('sets the config for pivot tables and only includes the intended columns and ' +
    'correctly excludes the datefield when aggregation is applied', () => {
    const config = setTableConfig(mockConfig, mockTableWithPivot, selectedPivotWithAggregation,
      mockApiPivotData);
    ...
    it('sets a width of 20 on every column except the last one, only if there are fewer ' +
      'than five columns', () => {
        const config = setTableConfig(mockConfig, selectedTableLessFields, {});
