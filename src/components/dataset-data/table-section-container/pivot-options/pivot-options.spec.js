...
describe('PivotOptions component does render children', () => {
  ...
  const selectedTable = {
    dataDisplays: [
      {"chartType": null, "dimensionField": "devs_only", "title": "Top Secret",
        "uniquePivotValues": [{"columnName": "red_shirts", "prettyName": "Expendable Star Trek Crew"},
          {"columnName": "jedis_are_better", "prettyName": "Star Wars > Star Trek"}]
      },
      {"chartType": null, "dimensionField": "additional_columns", "title": "Additional Columns",
        "uniquePivotValues": [{"columnName": "cheesy_title", "prettyName": "Gouda to Meet You"},
  ...
  it('creates two select/dropdown controls and two text labels', () => {
    ...
    expect(pivotViewSelector.props.options.map(opt => opt.title)).toEqual(["Top Secret", "Additional Columns", "By Facility", "By Form", "Complete Table"]);
  ...
  it('uses defaulted data table pivot values when pivot view does not provide its own pivot options', () => {
    ...
    expect(valueSelectControl.props.options.sort((a,b) => a.columnName.localeCompare(b.columnName)).map(opt => opt.columnName)).toStrictEqual(selectedTable.valueFieldOptions);
