...
export const placeTablePivots = selectedTable => {
  ...
  if (selectedTable && selectedTable.fields) {
    ...
    if (dataDisplays && dataDisplays.length > 1) {
      pivotViewsArr = dataDisplays
        .filter(pivot => pivot.dimensionField)
        .map(pivot => {
          const field = Object.assign(
            {},
            tableFields.find(p => p.columnName === pivot.dimensionField)
          );
    ...
    const pivotValuesArr =
      valueFieldOptions && valueFieldOptions.length
        ? valueFieldOptions.map(pivot => tableFields.find(p => p.columnName === pivot)).filter(el => el)
        : [];

    let pivotBankArr = selectedTable.fields.filter(field => !excludedFields.some(excluded => excluded === field.dataType));
    const srcLineNbrIdx = pivotBankArr.findIndex(pivot => pivot.columnName === 'src_line_nbr');
    if (srcLineNbrIdx !== -1) {
      pivotBankArr.splice(srcLineNbrIdx, 1);
    }

    pivotBankArr = pivotBankArr.filter(pivot => !pivotValuesArr.some(pVal => pVal.columnName === pivot.columnName));
...
const testHelperValueFieldOptions = [
  // Existing setup in prod as of 3/24/21
  ['interest_rate_pct', 'outstanding_advance_bal', 'advance_auth_month_amt', 'gross_advance_draws_month_amt'],
