...
const createPivotFields = (apiData, dateField, pivot) => {
  ...
  if (pivot && pivot.pivotView && pivot.pivotView.aggregateOn) {
    ...
    if (pivot.pivotView.title === 'Complete Table') {
      fieldsToUse = fieldsToUse
        .filter(k => !GLOBALS.DATE_RELATED_META_TYPES.includes(apiData.meta.dataTypes[k]));
    }
  } else {
    fieldsToUse = Object.keys(apiData.meta.labels);
  }
  return fieldsToUse.map(k => {
      return {
        columnName: k,
  ...
  }).sort((a, b) => { // in case of pivot, sort columns alphabetically for consistency
     if(!a || !b){
       if(a){
         return 1;
       }
...
export const setTableConfig = (config, selectedTable, selectedPivot, apiData) => {
    let fields;
    if (apiData && apiData.data && apiData.data.length && apiData.meta && (selectedPivot &&
      (selectedPivot.pivotValue ||
        (selectedPivot.pivotView && selectedPivot.pivotView.aggregateOn !== null)))) {
