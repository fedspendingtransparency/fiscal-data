...
export const convertDate = date => {
  ...
  const stringDate = date !== null && date !== undefined ? date.replace(/-/g, '/') : format(new Date(), 'yyyy-MM-dd');
...
export const getNormalizedDate = dateStr => {
  const curDate = dateStr !== null && dateStr !== undefined ? dateStr.replace(/-/g, '/') : dateStr;
...
export const getPresetDateRange = (years, latest, earliest) => {
  ...
  const stringDate = earliest !== null && earliest !== undefined ? earliest.replace(/-/g, '/') : '1790/01/01';
...
export const parseTableSelectionFromUrl = (location, tables) => {
  if (Array.isArray(tables)) {
    if (location && location.pathname) {
      ...
      const tableInUrl = tableSegment ? tables.find(table => table.pathName === tableSegment) : false;
...
export const rewriteUrl = (table, slug, location) => {
  if (window && window.history && table) {
    const urlRewrite = `/datasets${slug}${table.pathName}${location && location.search ? location.search : ''}`;
...
export const thinDataAsNeededForChart = (dataRows, slug, dateField, selectedTable) => {
  if (slug === '/debt-to-the-penny/' || (slug === '/qtcb-historical-interest-rates/' && (!selectedTable || !selectedTable.isLargeDataset))) {
    let latestYearMonth = '0';
    const subset = dataRows
      .slice()
      .reverse()
      .reduce((acc, d) => {
