...
import {
  minimalConfig, config, mockApiData, latestDate, fivePrior, mockLocation,
...
describe('DatasetData', () => {
  ...
  it('selects the correct table when it is specified in the url', async () => {
    ...
    expect(setSelectedTableFromUrl).toHaveBeenCalledWith(mockConfig.apis[2])
  });
...
describe('DatasetData - Integration tests', () => {
  ...
  it(`contains a DataTableSelect component with defaulted props`, () => {
    expect(instance.findByType(DataTableSelect).props.apis).toBe(mockConfig.apis);
  });
  ...
  selected`, async () => {
    ...
    expect(instance.findByType(FilterAndDownload).props.selectedTable.tableName)
      .toBe(mockConfig.apis[1].tableName);
  });
  ...
  it(`raises state on setSelectedTable when the table is updated`, async() => {
    await updateTable('Table 5');
    expect(setSelectedTableMock).toHaveBeenCalledWith(mockConfig.apis[4]);
  });
  ...
  it(`grabs the published reports from the publishedReports prop if the dataset is whitelisted`,
    async () => {
      ...
      const origId = mockConfig.datasetId;
      ...
      mockConfig.publishedReports = mockPublishedReports;
      if (whiteListIds && whiteListIds.length) {
        mockConfig.datasetId = whiteListIds[0];
      }
...
describe('Dataset data - configStore updates captured', () => {
  beforeAll(() => {
    configStore.dispatch({
      type: 'config',
      value: config
  ...
  it('captures updates to config', () => {
    const component = renderer.create(<DatasetDataComponent
      width={500}
      setSelectedTableProp={jest.fn()}
    />);
    const instance = component.root;
    jest.runAllTimers();
    let dataTableSelect = instance.findByType(DataTableSelect);
    expect(dataTableSelect.props.earliestDate).toEqual(config.techSpecs.earliestDate);
    ...
    jest.runAllTimers();
    dataTableSelect = instance.findByType(DataTableSelect);
    expect(dataTableSelect.props.earliestDate).toEqual(minimalConfig.techSpecs.earliestDate);
  });
});
