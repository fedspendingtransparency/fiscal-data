...
describe('DatasetData', () => {
  ...
  it(`contains a DataTableSelect component with defaulted props`, async () => {
    expect(instance.findByType(DataTableSelect).props.apis).toBe(config.apis);
  });

  it(`initializes the selected table to the first element in the apis array`, async () => {
  ...
  paginated data`, async () => {
    ...
    expect(callsToApiForUpdatedTable.length).toEqual(6);
  });
