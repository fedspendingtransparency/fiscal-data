...
import { buttonGroup, toggleButton, selected, buttonDescription, radio } from './report-data-toggle.module.scss';
...
const ReportDataToggle = ({ onChange, reports }) => {
  ...
  const buttons = [
    {
      ...
      value: 1,
      icon: <FontAwesomeIcon classes={faStyles.rawData} icon={faTable} data-test-id="table-icon" />,
    },
    {
      description: 'Published Reports',
      value: 2,
      icon: <FontAwesomeIcon classes={faStyles.publishedReport} icon={faFileDownload} data-test-id="report-icon" />,
    },
  ];
  ...
  return (
    <div className={buttonGroup} data-toggle="buttons" style={{ display: display }}>
      {buttons.map(radioBtn => (
        <label key={`report-toggle-${radioBtn.value}`} className={`${toggleButton} ${activeState === radioBtn.value ? selected : ''}`}>
          {radioBtn.icon}
