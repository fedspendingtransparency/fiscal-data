...
describe('TableCache Utility Class', () => {
  ...
  it(
    'exposes a function that compares cached record date ranges with a newly requested date range and returns an ' +
    ...
    () => {
      ...
      expect(beforeAfterAndInBetween).toEqual([
        '2021-03-22 - 2021-04-28',
        '2017-10-22 - 2017-10-27',
        '2015-09-08 - 2016-02-13',
        '2015-01-01 - 2015-02-13',
      ]);
  ...
  it('it returns a record set that includes a single array of data rows filtered to a ' + 'given dateRange', () => {
    ...
    expect(tableCache.getRecordSetForRange(mockRequestedRangeFullSegs, 'record_date').data).toEqual([
      ...mockDataCache[0].data,
      ...mockDataCache[1].data,
    ]);
    ...
    expect(tableCache.getRecordSetForRange(mockRequestedRangeFilterByRow, 'record_date').data).toEqual([
      mockDataCache[0].data[1],
      mockDataCache[1].data[0],
    ]);
  });

  it('caches multiple record set segments for a given table in date-decending order', () => {
    const tableCacheMock = new TableCache();
    tableCacheMock.updateDataCache([mockDataCache[3], mockDataCache[2], mockDataCache[1], mockDataCache[0]]);
  ...
  returns appropriate responses from dataDisplay cache`, () => {
    ...
    expect(tableCache.dataDisplayCache).toEqual({
      '2015-01-01:2021-04-28': {
        'By Desc:interest_amt': {
          data: 'rows..',
          pivotApplied: 'By Desc:interest_amt',
        },
      },
    });
    ...
    expect(tableCache.getCachedDataDisplay(mockRequestedRange, mockSelectedPivot, {})).toEqual({
      data: 'rows..',
      pivotApplied: 'By Desc:interest_amt',
    });
    ...
    expect(tableCache.getCachedDataDisplay(mockRequestedRange, mockSelectedPivot2, {})).toEqual({ data: 'more rows..' });
    ...
    expect(tableCache.getCachedDataDisplay(mockUncachedRange, mockSelectedPivot, {})).toBeFalsy();
  });
});
