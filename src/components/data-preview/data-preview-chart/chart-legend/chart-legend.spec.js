...
import { mockFields, mockFields2, mockFields6 } from '../chart-test-helper';

describe('Footer Chart Legend', () => {
  it('should render a checkbox for each field', () => {
    const { getByRole } = render(<ChartLegend fields={mockFields} onHover={jest.fn()} onLabelChange={jest.fn()} />);
  ...
  it('should call onLabelChange on key press', () => {
    ...
    const { getByRole } = render(<ChartLegend fields={mockFields} onHover={jest.fn()} onLabelChange={mockOnLabelChange} />);
  ...
  it('should set proper width for less then 4 fields', () => {
    const { getAllByTestId } = render(<ChartLegend fields={mockFields2} onHover={jest.fn()} onLabelChange={jest.fn()} />);
  ...
  it('should set proper width for 6 fields', () => {
    const { getAllByTestId } = render(<ChartLegend fields={mockFields6} onHover={jest.fn()} onLabelChange={jest.fn()} />);
    const checkboxContainer = getAllByTestId('legendField')[0];
