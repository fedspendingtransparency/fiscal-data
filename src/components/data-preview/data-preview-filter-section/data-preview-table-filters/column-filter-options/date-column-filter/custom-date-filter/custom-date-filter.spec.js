...
describe('Date column filter', () => {
  ...
  const mockColumnConfig2 = { defaultEndDate: new Date('3/16/2025'), defaultStartDate: new Date('3/17/2025') };
  ...
  it('correctly updates the start date', async () => {
    ...
    const newDateBox = getByRole('gridcell', { name: 2 });
    await userEvent.click(newDateBox);
    const applyButton = getByRole('button', { name: 'Apply Selected Date' });
    await userEvent.click(applyButton);
    within(startDateButton).getByText('test');
  });
});
