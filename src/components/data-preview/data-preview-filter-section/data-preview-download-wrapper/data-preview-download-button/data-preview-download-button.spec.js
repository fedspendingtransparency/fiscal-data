...
import DataPreviewDownloadButton from './data-preview-download-button';
...
describe('Data preview download button', () => {
  it('renders the desktop button default state', () => {
    const { getByRole } = render(<DataPreviewDownloadButton active={false} setActive={jest.fn()} width={1000} />);
  ...
  it('renders the mobile button default state', () => {
    const { getByRole } = render(<DataPreviewDownloadButton active={false} setActive={jest.fn()} width={500} />);
    expect(getByRole('button', { name: 'Download' })).toBeInTheDocument();
  ...
  it('renders the desktop button active state', () => {
    ...
    const { getByRole } = render(<DataPreviewDownloadButton active={true} setActive={setActiveSpy} width={1000} />);
  ...
  it('renders the mobile button active state', () => {
    const setActiveSpy = jest.fn();
    const { getByRole } = render(<DataPreviewDownloadButton active={true} setActive={setActiveSpy} width={500} />);
    const button = getByRole('button', { name: 'Download' });
    userEvent.click(button);
