...
const Endpoints = () => {
  ...
  result.allDatasets.datasets.sort((a,b) => {
    /*
        ...
        endpoint = (
          <span key={`wb${j}`}>
            /{wordBreakEndpoint.map((d,i) => (
              (i < wordBreakLen - 1) ? <span key={`wb${j}${i}`}>{d}/<wbr></wbr></span> : d
            ))}
          </span>
        );
      }
    ...
    <div className={apiStyles.sectionBreak}>
      ...
      >
        <p id="list-of-endpoints-id">
          ...
          </strong> of the corresponding endpoint.
        </p>
        <p>
      ...
      >
        <p>
          ...
          corresponding dataset’s details page for dataset-specific API documentation, or refer to
          its data dictionary.
        </p>
        <p>
