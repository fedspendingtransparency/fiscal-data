...
export class MultichartRenderer {
  ...
  setWidth = (selection: Selection<BaseType, unknown, HTMLElement, unknown>): void => {
    ...
    this.chartDimensions.width = this.w - this.chartDimensions.yAxisWidth - this.chartDimensions.marginRight;
  };

  setScales = (config: ChartConfig, chartCount: number, chartIndex: number): { x: unknown; y: unknown } => {
    ...
    if (extent[0] !== undefined && extent[1] !== undefined) {
      scales.y = d3
        .scaleLinear()
        .domain(extent)
        .range(config.options.inverted ? [config.segmentMinY, config.segmentMaxY] : [config.segmentMaxY, config.segmentMinY])
        .nice();
  ...
  draw(config: ChartConfig): void {
    const createArea = () =>
      area()
        ...
        .y1(d => config.scales.y(d[config.fields[0]]))(config.data.filter(d => d[config.fields[0]] && d[config.fields[0]] !== 'null'));

    if (config.options && config.options.shading) {
      config.lines = this.container
        .append('g')
        .selectAll('path')
        .data(config.fields);
  ...
  setContainer = (): void => {
    ...
    if (config.options.maxHeightToWidthRatio) {
      this.chartDimensions.height =
        this.chartDimensions.height / this.w <= config.options.maxHeightToWidthRatio
          ? this.chartDimensions.height
    ...
    this.container = parentSelection
      ...
      .attr('height', this.chartDimensions.height + this.chartDimensions.xAxisHeight + this.chartDimensions.marginTop)
      .attr('width', this.w)
      .append('g')
  ...
  addHoverEffects = (hoverFunction: (dateString: string | null) => void): void => {
    ...
    mouseTracker.on('click', this.mousemove).on('mousemove', this.mousemove);
  };
  ...
  mousemove = (): void => {
    ...
    closestXIndex = closestXIndex >= this.chartConfigs[0].data.length ? this.chartConfigs[0].data.length - 1 : closestXIndex;
  ...
  placeMarginLabels = (config: ChartConfig): void => {
    ...
    if (config.zeroMarginLabelLeft) {
      d3.select(elemClass)
        ...
        .attr(
          'transform',
          `translate(${config.scales.x(parseTime(config.data[config.data.length - 1][config.dateField])) - 12},
      ${config.scales.y(0) - 7})`
        );
    }
  ...
  placeMarker = (config: ChartConfig, dataIndex: number): void => {
    ...
    d3.select(elemClass)
      ...
      .attr(
        'transform',
        `translate(${config.scales.x(parseTime(config.data[dataIndex][config.dateField]))},
  ...
  clearMarkers = (configName: string): void => {
    this.container.selectAll(`.${configName}-marker`).remove();
    this.container.selectAll('.marker-connector').remove();
  };
}
