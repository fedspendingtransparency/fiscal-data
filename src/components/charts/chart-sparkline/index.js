...
let data, chartId, dateField, field, container, fontSize = 16, w, markers, hoverFunction;
const scales = {}
...
const placeMarkers = () => {
  ...
  container.select('.ink')
    ...
    .attr('transform', d => (
      `translate(${scales.x(parseTime(d[dateField]))}, ${scales.y(Number(d[field]))})`)
    )
}
...
const drawLineChart = ({ showXLabel, xStartLabel, xEndLabel, formatterXLabel }) => {
  ...
  fontSize = (d3.select('html').style('font-size')
    .replace(/px/g, ''));
