...
import {
  separateEntriesByDate,
  separateEntriesByName
} from '../calendar-entry-sort-helper/calendar-entry-sort-helper';
import * as styles from './calendar-entry-pages.module.scss';

const CalendarEntryPages = ({
  entries,
  activePage,
  selectedOption,
  entriesPerPage,
  ...
  for (const group of chunkedEntries) {
    const separatedEntries = selectedOption.id === 'name' ?
      separateEntriesByName(group) : separateEntriesByDate(group);
