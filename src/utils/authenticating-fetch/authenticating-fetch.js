...
module.exports = (authKey = paramsRequired(), fetchUtil = paramsRequired()) => (url, options) => {
  ...
  if (options.headers) {
    if (options.headers.set) { // use formal Headers object property setter if available
      options.headers.set('Authorization', `Basic ${authKey}`);
    } else { // otherwise treat headers as simple object
      options.headers['Authorization'] = `Basic ${authKey}`;
    }
  } else {
    if (typeof Headers !== 'undefined') {  // formal Headers is available so use it and its setter
      options.headers = new Headers();
      options.headers.set('Authorization', `Basic ${authKey}`);
    } else { // just use simple object
      options.headers = {'Authorization': `Basic ${authKey}`}
    }
  }
