 ...
 */
export const fetchHighlights = (endpoint, filters, fields, dateField,
                                limit, sorts, doNotAddDateFieldToFields) => {
  if (doNotAddDateFieldToFields === undefined
    || doNotAddDateFieldToFields === null) {
    doNotAddDateFieldToFields = false;
  }
  const filterString = (filters && Object.keys(filters).length)
    ? `&filter=${serializeFilters(filters)}` : '';
  const fieldsString =
    fields !== undefined
      ? `fields=${fields.join()}${!doNotAddDateFieldToFields ? `,${dateField}` : '' }`
      : '';
  const sortsString = `sort=${sorts !== undefined ? `${sorts.join(',')}` : `-${dateField}`}`;
  const url = `${apiPrefix}${endpoint}?format=json${filterString}&${fieldsString}&${sortsString}`
    + `&page[size]=${limit}`;
...
const checkError = (response, urlAttempted) => {
  if (!response.headers.get('Content-Type')
    || (
      ...
      && response.headers.get('Content-Type').toLocaleLowerCase().indexOf('json') === -1
    )) {
...
export const pagedDatatableRequest = async (table, from, to, selectedPivot, pageNum, pageSize) => {
  ...
  const uri = `${apiPrefix}${table.endpoint}?filter=${dateField}:gte:${fromStr},${dateField}`
    + `:lte:${toStr}&sort=${sortParam}&page[number]=${pageNum}&page[size]=${pageSize}`;

  return getIFetch()(uri).then(response => response.json());
};

export const datatableRequest = async (table, dateRange, selectedPivot,
                                       canceledObj, tableCache) => {
  ...
  if (pivotView && pivotView.dimensionField && pivotValue && pivotView.aggregateOn) {
    const pivotedData = await fetchPivotData(table, formatDateForApi(dateRange.from),
      formatDateForApi(dateRange.to), pivotView, pivotValue.columnName, pivotView.aggregateOn,
      canceledObj);
    if (tableCache) {
    ...
    const fieldsParam = (pivotView && pivotView.fields && pivotView.fields.length)
      ? `&fields=${pivotView.fields.join()}` : '';
    ...
    if (dateRanges && dateRanges.length) {
      ...
      dateRanges.forEach(range => {
        ...
        const uri = `${apiPrefix}${endpoint}?filter=${dateField}:gte:${from},${dateField}`
          + `:lte:${to}${fieldsParam}&sort=${sortParamValue}`;
        fetchers.push(fetchAllPages(uri, canceledObj).then(res => {
          res.range = range;
          return res;
        }));
      });
...
export const fetchAllPages = async (uri, canceledObj) => {
  ...
  if (!canceledObj.isCanceled) {
    ...
    finalData = await iFetch(`${uri}${nextPageParams}`,
      { signal: aborter.signal }).then(resp => resp.json());
    if (finalData && Array.isArray(finalData.data)) {
      ...
      if (totalPages > 1) {
        ...
        while (pageCount < totalPages && !canceledObj.isCanceled) {
          ...
          pagedResponseRecordSets[pageCount - 2] = iFetch(`${uri}${nextPageParams}`,
...
export const fetchPivotData = async (table, from, to, pivotView, pivotValueField,
                                     aggregateOn, canceledObj) => {
  const endpoint = table.endpoint;
  const dateField = table.dateField;
  ...
  return fetchAllPages(uri, canceledObj)
    .then(data => pivotData(data, dateField, pivotView, pivotValueField,
      aggregateOn, from, to, postFetchFilterFunction));
};
...
export const pivotData = (data, dateField, pivotView, pivotValueField, aggregateOn, from,
                          to, filterFn) => {
  const indexed = {};
  const labels = {};
  const dataTypes = {};
  ...
  // string literal
  const isEmptyPivotLabel = value => value === undefined || value === null
    || value === '' || ('' + value).toLocaleLowerCase() === 'null';

  let pivotColumnKey;

  if (aggregateOn && aggregateOn.length) { /* create composite column in case aggregation
  ...
  dataRows.forEach((row) => {
    ...
    if (aggregateOn && aggregateOn.length) {
      if (aggregateOn[0].type !== 'DATE') {
        rowKey = aggregateOn.map(ag =>
          formatDateSegment(row[ag.field], ag.type)).reverse().join(' ');
      } else {
    ...
    if (!isEmptyPivotLabel(row[dimensionField])) {  /* only make columns when the pivot
      ...
      if (!data.meta.dataTypes[pivotValueField].includes('CURRENCY')
        ...
        if (isNaN(Number(indexed[rowKey][row[dimensionField]]))) {
          ...
          // numeric, so sum them
          indexed[rowKey][row[dimensionField]] =
            Number(indexed[rowKey][row[dimensionField]]) + Number(row[pivotValueField]);
        }
  ...
  let pivotedDataRows;
  if (aggregateOn && aggregateOn.length) {
    pivotedDataRows = Object.values(indexed).sort((a, b) =>
...
export const incorporateChartDates = (data, pivotView) => {
  ...
  if (!aggregateOn || !aggregateOn.length) {
    ...
    data.data.forEach((row) => {
      ...
      Object.entries(row).forEach(([key, val]) => {
        if (!GLOBALS.DATE_RELATED_META_TYPES.includes(data.meta.dataTypes[key])) {
          if ((indexed[rowKey][key] === undefined) ||
            (data.meta.dataTypes[key] !== 'CURRENCY' && data.meta.dataTypes[key] !== 'NUMBER' &&
            !pivotView.lastRowSnapshot)) {
            indexed[rowKey][key] = val;
          } else if (!isNaN(Number(indexed[rowKey][key])) &&
            !isNaN(Number(val)) && !pivotView.lastRowSnapshot) {
    ...
    return {
      data: Object.values(indexed).sort((a, b) =>
        ('' + b['CHART_DATE']).localeCompare(a['CHART_DATE'])),
      meta: {
        labels,
        dataTypes
...
export const formulateUrl = (endpointPath, filters, fields, limit, sort, format) => {
  ...
  if (filters) {
    ...
    params.filter = (typeof(filters) === 'string' || filters instanceof String)
...
export const serializeFilters = (filters) => {
  ...
  filters.forEach(filter => {
    ...
    if (filter.operator === 'between') {
      param += `${filter.key}:${filter.range.low.inclusive ? 'gte' : 'gt'}`
        + `:${filter.range.low.value}`;
      param += `,${filter.key}:${filter.range.high.inclusive ? 'lte' : 'lt'}`
        + `:${filter.range.high.value}`;
...
export const buildSortParams = (table, _selectedPivot) => {
  ...
  if (pivotView && pivotView.aggregateOn && pivotView.aggregateOn.length
    && !pivotView.lastRowSnapshot) {
