...
describe('Api Utils function library', () => {
  ...
  it('adds sort parameters if alwaysSortBy property is provided', async () => {
    ...
    await datatableRequest(testTable, mockRange, selectedPivot, { isCanceled: false, abortController: { signal: null } }, new TableCache());
  ...
  it(
    'has a fetchAllPages method that automatically calls as ' +
  ...
  it('correctly applies alwaysSortWith fields in paginated requests', async () => {
    ...
    await pagedDatatableRequest(unitTestObjects.dataTables[1], '2021-01-01', '2021-02-01', null, 1, 5);
  ...
  it('applies descending sort order to paginated requests where ' + 'alwaysSortWith is not provided', async () => {
    ...
    const fetchSpy = jest.spyOn(global, 'fetch');
    await pagedDatatableRequest(unitTestObjects.dataTables[0], '2021-01-01', '2021-02-01', null, 1, 5);
  ...
  it('includes alwaysSort fields in th fields list for pivot requests', () => {
    ...
    expect(buildFields(mockTable, mockPivotValue, mockPivotValueField, mockAggregateOn).sort()).toStrictEqual(
      ['column1', 'column2', 'column3', 'column4'].sort()
    );
  });
