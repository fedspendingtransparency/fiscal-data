...
describe('API Utils Helper', () => {
  ...
  it('returns a valid request object when calling buildDownloadObject', () => {
    ...
    const returnObj = {
      apiId: 1,
      params: `?filter=${apiParam.dateField}:gte:${dateRange.from},${apiParam.dateField}`
        + `:lte:${dateRange.to}&sort=-${apiParam.dateField}&format=${fileType}`
    };

    expect(unitTestObjects.buildDownloadObject(apiParam, dateRange, fileType))
      .toStrictEqual(returnObj);
  });
  ...
  it('formats the date strings to YYYY-MM for specific APIs', () => {
    ...
    const returnObj = {
      ...
      params: `?filter=${apiParam.dateField}:gte:${formattedDateRange.from},${apiParam.dateField}`
  ...
  it('returns a valid request object from buildDownloadObject when called with api object ' +
    'has an alwaysSortWith attribute', () => {
    const apiParam = {
      apiId: 1,
  ...
  it('returns a valid request object when calling buildDownloadRequestArray with a ' +
    'single API', () => {
    const apiParam = {apiId: 1, dateField: 'record_date'};
    ...
    const returnObj = {
      apis: [
        {
          apiId: 1,
          params: `?filter=${apiParam.dateField}:gte:${dummyDateStr},${apiParam.dateField}`
            + `:lte:${dummyDateStr}&sort=-${apiParam.dateField}&format=${fileType}`
        }
      ]
    };

    expect(helpers.buildDownloadRequestArray(apiParam, dateRange, fileType))
      .toStrictEqual(returnObj);
  });

  it('returns a valid request object when calling buildDownloadRequestArray with an ' +
    'array of APIs', () => {
    const apiParams = [
    ...
    const returnObj = {
      apis: [
        {
          apiId: 3,
          params: `?filter=${apiParams[2].dateField}:gte:${dummyDateStr},${apiParams[2].dateField}`
            + `:lte:${dummyDateStr}&sort=-${apiParams[2].dateField}&format=${fileType}`
        },
        {
          apiId: 2,
          params: `?filter=${apiParams[1].dateField}:gte:${dummyDateStr},${apiParams[1].dateField}`
            + `:lte:${dummyDateStr}&sort=-${apiParams[1].dateField}&format=${fileType}`
        },
        {
          apiId: 1,
          params: `?filter=${apiParams[0].dateField}:gte:${dummyDateStr},${apiParams[0].dateField}`
            + `:lte:${dummyDateStr}&sort=-${apiParams[0].dateField}&format=${fileType}`
        }
      ]
    };

    expect(helpers.buildDownloadRequestArray(apiParams, dateRange, fileType))
      .toStrictEqual(returnObj);
  });
