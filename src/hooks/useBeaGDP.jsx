...
import { useVerifyAdditionalChartData } from '../helpers/hook-helpers/use-verify-additional-chart-data';
const useBeaGDP = (cpiData, inflationAdjust, otherDataToCheck) => {
  ...
  const { getDebtOutstanding, getMTS4, getMTS5 } = useVerifyAdditionalChartData();
      ...
      for (const gdp of beaData) {
        ...
        if (year === curYear) {
          ...
          if (year >= 1977 && allQuartersForGivenYear.find(entry => entry.timePeriod.includes(year.toString() + 'Q3'))) {
            ...
            let otherDataPresent;
            if (otherDataToCheck === 'debtOutstanding') {
              otherDataPresent = await getDebtOutstanding();
            } else if (otherDataToCheck === 'mts4') {
              otherDataPresent = await getMTS4();
            } else if (otherDataToCheck === 'mts5') {
              otherDataPresent = await getMTS5();
            }
            if (otherDataPresent) {
              let totalGDP = 0;
              allQuartersForGivenYear.forEach(quarter => {
