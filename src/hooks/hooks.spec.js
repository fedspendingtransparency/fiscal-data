...
describe('useBEAGDP', () => {
  ...
  it('First and last entry of finalGDPData after data render will match mocked values given', () => {
  ...
  it('1952 and 1984 data is adjusted according to mocks', () => {
    const { result } = renderHook(() => useBEAGDP(mockCpiDataset, true));
    expect(
...
describe('useBEAGDP Q3 senario', () => {
  ...
  it('GDP calc uses average of Q4-Q2 of current year if no Q3 when other data is in for the current year', async () => {
  ...
  it('GDP calc does not use current year if no Q3 when other data is not in for the current year', async () => {
    StaticQuery.mockImplementation(({ render }) => render({ mockNoQ3BEAData }));
    useStaticQuery.mockImplementation(() => {
      return {
        ...mockNoQ3BEAData,
    ...
    const { result } = renderHook(() => useBEAGDP(mockCpiDataset, false, 'mts5'));
    await waitFor(() => expect(result.current.finalGDPData[result.current.finalGDPData.length - 1].fiscalYear).toBe('1984'));
  ...
  it('GDP calc uses current year if there is current year Q3', () => {
    StaticQuery.mockImplementation(({ render }) => render({ mockBEAData }));
    useStaticQuery.mockImplementation(() => {
      return {
        ...mockBEAData,
  ...
  it('GDP calc does not use current year when no Q3 and otherDataPresent is not flagged', () => {
    StaticQuery.mockImplementation(({ render }) => render({ mockNoQ3BEAData }));
    useStaticQuery.mockImplementation(() => {
      return {
        ...mockNoQ3BEAData,
...
describe('useGAEventTracking', () => {
  it('Gets proper GA Event Info based on csv and event number', () => {
  ...
  it('getGAEvent returns null if no event number given', () => {
    StaticQuery.mockImplementation(({ render }) => render({ mockGAEventCSVs }));
    useStaticQuery.mockImplementation(() => {
      return {
        ...mockGAEventCSVs,
  ...
  it('Gets proper GA Event Info based on csv and event number with a dynamic value', () => {
    StaticQuery.mockImplementation(({ render }) => render({ mockGAEventCSVs }));
    useStaticQuery.mockImplementation(() => {
      return {
        ...mockGAEventCSVs,
