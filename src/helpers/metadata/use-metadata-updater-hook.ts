...
export const useMetadataUpdater = (
  context: Record<string, unknown | IDatasetConfig> | IDatasetConfig[]
) => {
  ...
  useEffect(() => {
    ...
    if (isDatasetPageContext(context)) {
      processDatasetPageContext(context as Record<string, unknown | IDatasetConfig>, summaryData)
        .then((fullyUpdatedContext) => {
  ...
  useEffect(() => {
    ...
    updateSub = metadataService.summaryUpdated()
      .subscribe((sumData: ISummaryData) => {
...
const processDatasetPageContext = async (
  ...
  updatedDatasetConfig['apis'] =
    updateDatasetApiData(
      updatedDatasetConfig['apis'],
      summaryData[updatedDatasetConfig.datasetId]
    );

  updatedDatasetConfig['techSpecs'] =
    generateTechSpecs(
      updatedDatasetConfig['techSpecs'],
      summaryData[updatedDatasetConfig.datasetId]
    );
...
const updateDatasetApiData = (datasetConfigAPIs: IDatasetApi[],
                              summaryData: ISummaryDatasetData): IDatasetApi[] => {
...
const processDatasetsData = (datasetsData: IDatasetConfig[],
                             summaryData: ISummaryData): IDatasetConfig[] => {
  ...
  datasetsData.forEach((dataset: IDatasetConfig, index: number, arr: IDatasetConfig[]) => {
    ...
    if (summaryData[datasetId]) {
      datasets[index] = {
        ...dataset,
        techSpecs: generateTechSpecs(
          arr[index]['techSpecs'],
          summaryData[datasetId]
        )
      };

      // datasets may not have filters (related datasets on detail page)
      if (arr[index]['filters']) {
        datasets[index]['filters'] = updateDatasetFilters(
          arr[index]['filters'],
          summaryData[datasetId].lastUpdated
        )
      }
    }
  });
...
const updateDatasetFilters = (currentFilters: string[],
                              updatedLastUpdatedDate: Date): string[] => {
  ...
  if (filtersToRemove.length > 0) {
    output = output.filter((filter: string) => { return filtersToRemove.indexOf(filter) < 0 } );
  }
...
const generateTechSpecs = (currentTechSpecs: IDatasetTechSpecs,
                           updatedSummaryData: ISummaryDatasetData): IDatasetTechSpecs => {
  return {
    ...currentTechSpecs,
...
const updatePublishedReports = async (datasetConfig: IDatasetConfig): Promise<IDatasetConfig> => {
  return new Promise<IDatasetConfig>((resolve) => {
    metadataService.updatedPublishedReports(datasetConfig['datasetId'])
      .pipe(take(1))
      .subscribe({
