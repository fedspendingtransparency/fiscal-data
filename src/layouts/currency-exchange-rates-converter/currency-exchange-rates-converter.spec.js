...
describe('labelIcon', () => {
  ...
  it('returns correct JSX when an icon is provided', () => {
    ...
    const {queryByText, container } = render(labelIcon(label, iconName, true));

    expect(queryByText(label)).toBeTruthy();
    expect(container.querySelector('.info-tip-icon') || container.querySelector('[data-testid="infoTipButton"]')).not.toBeNull();
  });

});
describe('updateCurrencyDropdownOptions', () => {
  it('updates the dropdown options based on selected year and quarter', () => {
    const { getByTestId, rerender } = render(<CurrencyExchangeRatesConverter />);

    fireEvent.change(getByTestId('year-selector'), { target: { value: '2023' } });
    fireEvent.change(getByTestId('quarter-selector'), { target: { value: 'Q1' } });


    expect(getByTestId('currency-dropdown').options.length).toBeGreaterThan(0); 
