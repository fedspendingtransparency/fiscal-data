...
const PictorialChart = () => {
  ...
  const allYearKeys = data.reduce((keys, yearData) => {
    const keysForYear = Object.keys(yearData).filter((key) => key !== 'year');
    return [...keys, ...keysForYear];
  }, []);

  const renderedBars = allYearKeys
