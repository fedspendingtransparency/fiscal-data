  ...
  useEffect(() => {
    ...
    basicFetch(`${apiPrefix}${endpointURL}`).then(res => {
      if (res.data[0]) {
        ...
        basicFetch(`${apiPrefix}${supplementaryEndpointURL}`).then(supplementaryRes => {
          if (supplementaryRes.data[0]) {
            setIndvPercent(
              (
                Math.round(
                  (parseFloat(res.data[0]?.current_fytd_rcpt_outly_amt) / parseFloat(supplementaryRes.data[0]?.current_fytd_rcpt_outly_amt)) *
                    100 *
                    10
                ) / 10
              ).toFixed()
            );
            basicFetch(`${apiPrefix}${socialSecurityEndpointURL}`).then(socSecRes => {
              if (socSecRes.data[0]) {
                ...
                setSSPercent(
                  (Math.round((combinedSocialSecurity / parseFloat(supplementaryRes.data[0]?.current_fytd_rcpt_outly_amt)) * 100 * 10) / 10).toFixed()
                );
              }
            });
          }
    ...
    <div className={sourcesContent}>
      ...
      <div className={revenueAccordion}>
        ...
        >
          ...
          States, requires that any excess earnings generated by 12 Federal Reserve Banks be paid to the federal government. In practice, Federal
          Reserve banks pay for their operating expenses, pay a dividend to stockholders (U.S. commercial banks), set aside money in a surplus fund
          (capped at $6.8 billion), and the remainder is “transferred…to the Secretary of the Treasury for deposit in the general fund of the
          Treasury.” [<i>See</i> {federalReserveAct}]
        </Accordion>
      </div>
      <h5>Social Security and Medicare Taxes</h5>
      <p>
        Unlike personal income taxes, which support a variety of programs, these taxes are only used to fund Social Security and Medicare. These funds
        are collected from your paycheck, and in most cases, matched by your employer, and then divided into separate {trustFunds} that support each
        of those programs.
      </p>
      <p>
        Social Security has two trust fund accounts: the Old Age and Survivors Insurance Trust Fund (OASI) and the Disability Trust Fund (DI). The
        funds in these accounts are responsible for providing workers and their families with retirement, disability, and survivor's insurance
        benefits.
      </p>
      <p>
        Medicare also has two accounts: the Hospital Insurance Trust Fund (HI), also known as Medicare Part A, and the Supplementary Medicare
        Insurance Trust Fund (SMI). These funds pay for hospital, home health, skilled nursing, and hospice care for the elderly and disabled.
      </p>
      <QuoteBox
