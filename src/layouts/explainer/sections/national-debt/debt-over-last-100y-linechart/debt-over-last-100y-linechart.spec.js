...
describe('National Debt Over the Last 100 Years Chart', () => {  
  ...
  it("renders the calloutText", async () => {
    ...
    expect(await getByText("Over the past 100 years, the U.S. federal debt has increased from $410 B in 1922 to $30.93 T in 2022.", { exact: false })).toBeInTheDocument();
  });
  ...
  it("renders the chart markers and data header labels", async () => {
    ...
    await waitFor(() => expect(fetchSpy).toBeCalled());
    expect(await getAllByText("Total Debt")).toBeInTheDocument();
  ...
  it("renders the chart headers", async () => {
    ...
    await waitFor(() => expect(fetchSpy).toBeCalled());
    expect(await getByText("U.S. National Debt Over the Last 100 Years", { exact: false })).toBeInTheDocument();
