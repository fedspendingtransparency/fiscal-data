...
describe('Breaking Down the Debt', () => {
  ...
  beforeEach(() => {
    ...
    setGlobalFetchMatchingResponse(jest, [
      ...
      {
        matcher: url => {
          return url.includes('avg_interest_rates');
        },
      ...
      {
        matcher: url => {
          return url.includes('mts_table_5?filter');
        },
        jsonResponse: mockDebtExpenseResponse,
      },
    ]);
  ...
  it('makes the api call with a correctly configured query string', async () => {
    ...
    render(<BreakingDownTheDebt sectionId={sectionId} glossary={glossary} />);

    const queryString =
      '?fields=debt_held_public_mil_amt,intragov_hold_mil_amt,' +
  ...
  it('contains a header for the multichart with correct default values', async () => {
    const { getByTestId } = render(<BreakingDownTheDebt sectionId={sectionId} glossary={glossary} />);
