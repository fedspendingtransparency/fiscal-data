...
const BreakingDownTheDebt = ({ sectionId, width }) => {
  ...
  const { endpoint, getQueryString, transformer, multichart } = nationalDebtSectionConfigs[sectionId];
  ...
  const hoverEffectHandler = recordDate => {
    const year = recordDate === null ? multichartConfigs[0].data[0][multichartConfigs[0].dateField].substring(0, 4) : recordDate.substring(0, 4);
    if (multichartConfigs[0].data && multichartConfigs[1].data) {
      const interestRow = multichartConfigs[0].data.find(row => row[multichartConfigs[0].dateField].indexOf(year) === 0);
      const interest = percentageFormatter(interestRow[multichartConfigs[0].fields[0]]);

      const debtRow = multichartConfigs[1].data.find(row => row[multichartConfigs[1].dateField].indexOf(year) === 0);
  ...
  useEffect(() => {
    ...
    Promise.all(fetchers).then(() => {
      ...
      if (dataPopulatedConfigs[0].data) {
        setMultichartStartYear(dataPopulatedConfigs[0].data[dataPopulatedConfigs[0].data.length - 1].record_calendar_year);
  ...
  useEffect(() => {
    basicFetch(`${apiPrefix}${endpoint}${getQueryString()}`).then(response => {
      ...
      if (response && response.data && response.data.length) {
        ...
        basicFetch(`${apiPrefix}v1/accounting/mts/mts_table_5?filter=line_code_nbr:eq:4177&sort=-record_date&page[size]=1`).then(response => {
          if (response && response.data && response.data.length) {
            ...
            const maintainDebtExpense = parseFloat(response.data[0].current_fytd_net_outly_amt);
  ...
  return (
    <>
      <p>
        The national debt is composed of distinct types of debt, similar to an individual whose debt consists of a mortgage, car loan, and credit
        cards. The national debt can be broken down by whether it is non-marketable or marketable and whether it is{' '}
        {glossaryTerms.debtHeldByThePublic} or debt held by the government itself (known as {glossaryTerms.intragovernmental}). The national debt does
        not include debts carried by state and local governments, such as debt used to pay state-funded programs; nor does it include debts carried by
      ...
      <div className={postGraphContent} id="maintaining-national-debt">
        ...
        <p>
          The federal government is charged interest for the use of lenders’ money, in the same way that lenders charge an individual interest for a
          car loan or mortgage. How much the government pays in interest depends on the total national debt and the various securities’{' '}
                ...
                <div className={footerContainer}>
                  <p>
                    ...
                    <CustomLink
                      ...
                      onClick={() => analyticsClickHandler('Debt Citation Click', 'Interest Rate and Total Debt')}
