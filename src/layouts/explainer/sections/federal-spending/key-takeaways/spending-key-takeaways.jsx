...
const SpendingKeyTakeaways = ({ glossary, glossaryClickHandler }) => {
  ...
  useEffect(() => {
    ...
    const beaEndpointUrl =
      'https://apps.bea.gov/api/data/?UserID=F9C35FFF-7425-45B0-B988-' +
    ...
    basicFetch(`${apiPrefix}${endpointUrl}`).then(res => {
      if (res.data) {
        ...
        setPriorYearSpendingShort(getShortForm(priorYearSpending.toString(), false));

        basicFetch(`${beaEndpointUrl}`).then(bea_res => {
