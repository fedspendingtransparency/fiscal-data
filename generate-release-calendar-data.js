...
const generateReleaseCalendarData = () => {
  ...
  for (let i=0; i <= numberOfDaysToGenerate; i++) {
    ...
    byDayOfWeek[dayOfWeek].forEach((ds, index, array) => {
      ...
      } else if (ds.frequency === 'bw' &&
        (ds.lastDate === null || targetDate > addDays(lastDate, 8))) {
        rcDataArray[rcDataArray.length] = generateData(ds, targetDate);
        byDayOfWeek[dayOfWeek][index].lastDate = targetDate.toISOString();
      } else if (ds.frequency === 'm' &&
        (ds.lastDate === null || targetDate >= addDays(lastDate, 29))) {
        rcDataArray[rcDataArray.length] = generateData(ds, targetDate);
        byDayOfWeek[dayOfWeek][index].lastDate = targetDate.toISOString();
      } else if (ds.frequency === 'bm' &&
        (ds.lastDate === null || targetDate >= addDays(lastDate, 58))) {
        rcDataArray[rcDataArray.length] = generateData(ds, targetDate);
        byDayOfWeek[dayOfWeek][index].lastDate = targetDate.toISOString();
      } else if (ds.frequency === 'q' &&
        (ds.lastDate === null || targetDate >= addDays(lastDate, 89))) {
        rcDataArray[rcDataArray.length] = generateData(ds, targetDate);
        byDayOfWeek[dayOfWeek][index].lastDate = targetDate.toISOString();
      }
...
fs.writeFile('src/testData/release-calendar.mock.data.json', data,
  (error) => {
    if (error) throw error;
    const sd = new Date(argv.startDate);
    console.info("Release Calendar Mock Data Generated Successfully starting %s for %s days",
      sd.toISOString().split('T')[0], argv.daysToMock || 200);
  });
